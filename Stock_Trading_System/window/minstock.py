...
class getMinData:
    ...
    def getjusik(self):
        ...
        self.stockChart.SetInputValue(6, ord('m'))
        ...
        for i in range(numData):
            ...
            c1.append(str(self.stockChart.GetDataValue(1, i)))
        ...
        dataDf.to_csv('./minstockdata/'+self.name+' 분 단위 주식 데이터.csv', encoding='euc-kr', index=False)
        print('[시스템]:'+self.name+' 종목의 분 단위 주식 데이터 요청 수신성공 및 csv로 저장 성공!')

        print('[시스템]:'+self.name+' 종목의 분 단위 주식 데이터 백업본 csv 불러오는중...')
        backUpDataDf = pd.read_csv('./hapminstockdata/'+self.name+' 분 단위 주식 데이터.csv', encoding='euc-kr')
        hapData = pd.concat([backUpDataDf, dataDf], axis=0)
        hapData.to_csv('./hapminstockdata/'+self.name+' 분 단위 주식 데이터.csv', encoding='euc-kr', index=False)
        print('[시스템]:'+self.name+' 종목의 분 단위 주식 데이터 백업본 업데이트 성공!')